...
public class ClientApplication {
    ...
    // contains everything!
    public ClientApplication(byte[] seed, Key sharedKey, Key otherKey, int idx, byte[] tag, int otherIdx,
            byte[] otherTag, BulletinBoard board, Database db, int id) throws RemoteException {
    ...
    public ClientApplication(byte[] seed, Key intialKey, int initialIdx, byte[] initialTag, BulletinBoard board,
            Database db, int id) throws RemoteException {
        this.random = new SecureRandom(seed);
        ConnectionParams params = board.getConnectionParams();
        this.n = params.n;
        this.tagSize = params.tagSize;
    ...
    // get hash returns a hash made from sharedKey, idx, tag, otherKey, otherIdx,
    // otherTag
    private String getHash() {
        try {
            ...
            if (otherKey != null)
                digest.update(otherKey.getEncoded());
    ...
    public void receiveKeyTransferDTO(KeyTransferDTO dto) {
        ...
        try {
            ...
            KeyTransferDTO keyTransferDTO = new KeyTransferDTO(((SecretKeySpec) otherKey).getEncoded(), otherIdx,
                    otherTag);
     ...
     * BoardContent content = new BoardContent(message.getBytes(), idxNext, tagNext,
     * type);
     * 
     * // Encrypt the content
